...
select events.id, events.starttime, events.endtime,
    buckets.name, buckets.hostname, buckets.type, buckets.client,
    events.data,
...
from events INNER JOIN buckets
ON events.bucketrow=buckets.id
where buckets.type != "afkstatus";